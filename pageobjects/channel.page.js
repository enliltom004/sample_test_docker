...
class ChannelPage extends Page {
  ...
  get channel() {
    //const newName = this.setChannelName(this.name)
  ...
  async getChannel(value){
    await $(`[data-qa="channel_sidebar_name_${value}"]`).waitForDisplayed({timeoutMsg:`${value} is not loaded within 20s`})
  ...
  async saveMessage() {
    ...
    // await this.sandwitchButton.click();
  ...
  async clickSavedTab(){
    await this.savedTab.waitForDisplayed({timeoutMsg:'Saved Tab is not visible within 20s'})
  ...
  async validateResults (result) {
    //console.log("Visible " + this.emptyResult.isVisible() )
    let isVisible  = await this.emptyResult.isExisting();
    if (!isVisible ){
